object Advent02 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val passwords = inputs.collect {
      case regex(min, max, char, password) => PasswordPolicy(min.toInt, max.toInt, char.charAt(0)) -> password
    }

    // count the number of passwords that are valid according to the policy
    println(
      passwords.count { case (policy, password) =>
